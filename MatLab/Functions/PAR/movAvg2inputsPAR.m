function sh = movAvg2inputsPAR(x,data,scaling,cost,range,bigPoint,showBar)
...
i=1; smpl=1; contRow = 1;

if (row > 10)
    smplToc = zeros(10,1); 
    while smpl < 11 && i <= row
        if x(i,1) >= x(i,2)                  % ensures we don't dupe the optimizations.  Checks Fast > Slow
            sh(i) = NaN;
        else
            if col > 2
                type = x(i,3);
            ...
            [~,~,sh(i)] = movAvg2inputs(data,x(i,1),x(i,2),type,scaling,cost,bigPoint);
...
if contRow <= row
    ...
    if x(i,1) >= x(i,2)
        ...
        % We are not calling MEX because we want user feedback like progress bar.
        % Use movAvg2inputsPARMEX to run without feedback
        [~,~,sh(i)] = movAvg2inputs(data,x(i,1),x(i,2),type,scaling,cost,bigPoint); %#ok<PFBNS>
    end;
