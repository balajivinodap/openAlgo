...
parfor ii = 1:row
    if x(ii,1) > x(ii,2)
        ...
        [~,~,shTest(ii)] =	maRsiMETS(vBarsTest,x(ii,1),x(ii,2),x(ii,3),[x(ii,4) x(ii,5)],x(ii,6),...
                                                x(ii,7),x(ii,8),scaling,cost,bigPoint); 
        [~,~,shVal(ii)] =	maRsiMETS(vBarsVal,x(ii,1),x(ii,2),x(ii,3),[x(ii,4) x(ii,5)],x(ii,6),...
                                                x(ii,7),x(ii,8),scaling,cost,bigPoint);  %#ok<PFBNS>
    end
