function shMETS = rsiRaviPAR(x,data,scaling,cost,bigPoint)
...
parfor ii = 1:row
	[~,~,shTest(ii)] = rsiRavi(vBarsTest,[x(ii,1) x(ii,2)],x(ii,3),x(ii,4),x(ii,5),x(ii,6),x(ii,7),x(ii,8),...
                                x(ii,9), x(ii,10),scaling,cost,bigPoint); 
	[~,~,shVal(ii)] = rsiRavi(vBarsVal,[x(ii,1) x(ii,2)],x(ii,3),x(ii,4),x(ii,5),x(ii,6),x(ii,7),x(ii,8),...
