function shMETS = maSnrPARMETS(x,data,scaling,cost,bigPoint)
...
parfor ii = 1:row
    if x(ii,1) > x(ii,2)
        ...
        [~,~,shTest(ii)] =	maSnrSIG_DIS(vBarsTest,x(ii,1),x(ii,2),x(ii,3),x(ii,4),x(ii,5),...
                                                scaling,cost,bigPoint); 
        [~,~,shVal(ii)] =	maSnrSIG_DIS(vBarsVal,x(ii,1),x(ii,2),x(ii,3),x(ii,4),x(ii,5),...
