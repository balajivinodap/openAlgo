function shMETS = iTrendRaviPAR(x,data,scaling,cost,bigPoint)
...
parfor ii = 1:row
	[~,~,shTest(ii)] = iTrendRavi(vBarsTest,x(ii,1),x(ii,2),x(ii,3),x(ii,4),x(ii,5),x(ii,6),...
                                            scaling,cost,bigPoint); 
	[~,~,shVal(ii)] = iTrendRavi(vBarsVal,x(ii,1),x(ii,2),x(ii,3),x(ii,4),x(ii,5),x(ii,6),...
