function varargout = maRsiMETS(price,N,M,typeMA,Mrsi,thresh,typeRSI,isSignal,scaling,cost,bigPoint)
...
[sma,~,~,lead,lag] = movAvg2inputsMEX_mex(price,N,M,typeMA,scaling,cost,bigPoint);
[srsi,~,~,ri,ma,thresh] = rsiMETS(price,Mrsi,thresh,typeRSI,scaling,cost,bigPoint);
...
if nargout == 0
    ...
    hSub = cellstr(layout);
    movAvg2inputs(price,N,M,typeMA,scaling,cost,bigPoint,hSub);
    
    layout = ['6      ';'3      ';'[2 5]  ';'[8 11] ';'[14 17]'];
    hSub = cellstr(layout);
    rsiMETS(price,Mrsi,thresh,typeRSI,scaling,cost,bigPoint,hSub);
